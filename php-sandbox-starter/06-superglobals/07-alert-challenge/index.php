...
$description = '';
$submitted = false;
$messages_array = []; // Initialize messages array
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
  ...
  if (empty($title)) {
    $messages_array[] = ['text' => 'Title is required', 'color' => 'text-red-500']; // Use $messages_array instead of $messages
    $submitted = false;
  }

  if (empty($description)) {
    $messages_array[] = ['text' => 'Description is required', 'color' => 'text-red-500']; // Use $messages_array instead of $messages
    $submitted = false;
  }
  ...
  if ($file['error'] === UPLOAD_ERR_OK) {
    ...
    if (in_array($fileExtension, $allowedExtensions)) {
      ...
      if (move_uploaded_file($file['tmp_name'], $uploadDir .  $filename)) {
        $messages_array[] = ['text' => 'File uploaded successfully', 'color' => 'text-green-500']; // Use $messages_array instead of $messages
        $submitted = true;
      } else {
        $messages_array[] = ['text' => 'File Upload Error: ', 'color' => 'text-red-500']; // Use $messages_array instead of $messages
      }
    } else {
      $messages_array[] = ['text' => 'Must be an image', 'color' => 'text-red-500']; // Use $messages_array instead of $messages
    }
  }
}
?>
